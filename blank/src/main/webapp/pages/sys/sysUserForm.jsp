...
<script type="text/javascript">
	...
	var submitNow = function($dialog, $grid, $pjq) {
		...
		if ($(':input[name="data.id"]').val().length > 0) {
			url = datalook.contextPath + '/sysUser!update.action';
		} else {
 ...
 */	
	...
	$(function() {
		...
		if ($(':input[name="data.id"]').val().length > 0) {
			...
			$.post(datalook.contextPath + '/sysUser!getById.action', {
...
/* 					if (result.photo) {
						$('#photo').attr('src', datalook.contextPath + result.photo);
					}
 */				}
...
/* 		uploader = new plupload.Uploader({//上传插件定义
			...
			//flash_swf_url : datalook.contextPath + '/jslib/plupload_1_5_7/plupload/js/plupload.flash.swf',// Flash环境路径设置
			url : datalook.contextPath + '/plupload?fileFolder=/userPhoto',//上传文件路径
		...
		<fieldset>
			...
			<table class="table" style="width: 100%;">
				<tr>
					...
					<th>登陆帐号</th>
					<td><input name="data.username" class="easyui-validatebox" data-options="required:true,validType:'length[1,16]',invalidMessage:'长度不能超过16个字符'" /></td>
				</tr>
				<tr>
					<th>姓名</th>

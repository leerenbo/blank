	...
	var deleteByStatus = function() {
		...
		parent.${symbol_dollar}.messager.confirm('询问', '您确定要删除记录？', function(r) {
			if (r) {
				...
				${symbol_dollar}.post(ez.contextPath + '/sysRole!deleteByStatus.action', {
					'ids' : ids.join(',')
				}, function() {
	...
	<div id="eztoolbar">
		<div>
			<form id="searchForm">
				...
				<a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'ext-icon-zoom_out',plain:true" onclick="${symbol_dollar}('${symbol_pound}searchForm input.easyui-textbox').textbox('setValue','');ezgrid.datagrid('load',{});">清空查询</a>
			</form>
		</div>

		<div>
			<s:if test="@${package}.util.base.SecurityUtil@havePermission('/sysRole!save')">
				<a onclick="save();" href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'ext-icon-note_add',plain:true">添加</a>
			</s:if>
			<s:if test="@${package}.util.base.SecurityUtil@havePermission('/sysRole!getById')">
			...
			<s:if test="@${package}.util.base.SecurityUtil@havePermission('/sysRole!update')">
			...
			<s:if test="@${package}.util.base.SecurityUtil@havePermission('/sysRole!grantSysFunction')">
			...
			<s:if test="@${package}.util.base.SecurityUtil@havePermission('/sysRole!deleteByStatus')">

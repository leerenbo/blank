...
<script type="text/javascript">
	var selectIcon = function(${symbol_dollar}dialog, ${symbol_dollar}input) {
		${symbol_dollar}input.val(${symbol_dollar}(':radio:checked').val()).attr('class', ${symbol_dollar}(':radio:checked').val());
		${symbol_dollar}dialog.dialog('destroy');
	};
	${symbol_dollar}(function() {

		${symbol_dollar}(':radio').each(function(index) {//初始化小图标
			${symbol_dollar}(this).after('<img class="iconImg ' + ${symbol_dollar}(this).val() + '"/>');
		});
		${symbol_dollar}('.iconImg').attr('src', ez.pixel_0);

		${symbol_dollar}('td').click(function() {//绑定点击td事件，作用是点击td的时候，就可以选中，不一定非得点击radio组件
			${symbol_dollar}(this).find(':radio').attr('checked', 'checked');
		});

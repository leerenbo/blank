	...
	<head>
		...
		<script type="text/javascript">
			...
			${symbol_dollar}(function (){
						...
						[  
							...
							,
							{
								...
								formatter : function(value, row){
									var str = '';
									<s:if test="@${package}.util.base.SecurityUtil@havePermission('/employee!update')">
										str += ez.formatString('<img class="iconImg ext-icon-note_edit" title="编辑" onclick="editFun(${symbol_escape}'{0}${symbol_escape}');"/>', row.id);
									</s:if>
									<s:if test="@${package}.util.base.SecurityUtil@havePermission('/employee!deleteByStatus')">
										str += ez.formatString('<img class="iconImg ext-icon-note_delete" title="删除" onclick="removeFun(${symbol_escape}'{0}${symbol_escape}');"/>', row.id);
									</s:if>
									if (row.cards == "undefined" || row.cards == null || row.cards == undefined || row.cards == "") {
										<s:if test="@${package}.util.base.SecurityUtil@havePermission('/employee!saveArchivesOpenCard')">
											str += ez.formatString('<img class="iconImg ext-icon-vcard_add" title="有档案开卡" onclick="ArchivesOpenCard(${symbol_escape}'{0}${symbol_escape}');"/>', row.id);
										</s:if>
									}
									return str;
								}
				...
				/* ----------------------员工档案增加start---------------------- */
				${symbol_dollar}("${symbol_pound}addFun").click(function(){
				...
				function deleteOpenCardByStatus(row) {
					parent.${symbol_dollar}.messager.confirm('询问', '该员工已开卡，删除档案信息则同时删除卡户信息，继续请按确定', function(r) {
						if (r) {
							parent.${symbol_dollar}.messager.confirm('询问', '您确定要删除员工['+row.employeename+row.employeenumber+']的记录？', function(r) {
								if (r) {
									${symbol_dollar}.post(
				...
				/* ----------------------有档案开卡start---------------------- */
				${symbol_dollar}("${symbol_pound}archivesOpenCard").click(function() {
					//单行判断
					var rows = ${symbol_dollar}("${symbol_pound}grid").datagrid('getChecked');
					if (rows.length > 1) {
						${symbol_dollar}.messager.alert('提示', '请选择一条信息进行操作', 'info');
					...
					//是否已经开卡
					if (checkCardInfo(id)) {
						${symbol_dollar}.messager.alert('提示', '该员工已开卡，不能重复开卡', 'info');
						return;
					}
					var dialog = parent.ez.modalDialog({
						title : '有档案开卡',
			...
			function deleteByStatus(id) {
				parent.${symbol_dollar}.messager.confirm('询问', '您确定要删除此记录？', function(r) {
					if (r) {
						${symbol_dollar}.post(
							datalook.contextPath + '/employee!deleteByStatus.action', 
							{'data.id' : id}, 
					...
					<td>
						...
						<form id="searchForm">
							<table>
								<tr>
									<!-- 新增员工档案信息(权限控制) -->
									<s:if test="@${package}.util.base.SecurityUtil@havePermission('/employee!save')">
										<td>

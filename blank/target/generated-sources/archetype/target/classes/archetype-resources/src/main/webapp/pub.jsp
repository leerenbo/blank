...
<script type="text/javascript">
	...
	${symbol_dollar}(function (){
		${symbol_dollar}("${symbol_pound}deptTree").combotree({
			editable : false,
			url : '<%=contextPath%>/department!noSnSy_getDeptTreeData.action',
			checkbox : false,
			multiple : false,
			onSelect : function(node){
				var tree = ${symbol_dollar}(this).tree;//返回树对象
				var isLeaf = tree('isLeaf', node.target);
				/*
				if (!isLeaf) {
					${symbol_dollar}("${symbol_pound}deptTree").combotree('clear');//选中的节点是否为叶子节点。如果不是清除选中
				}
				*/
			}
		});

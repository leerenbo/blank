						...
						[  
							...
							,
							{
								...
								formatter : function(value, row){
									var str = '';
									<s:if test="@it.pkg.util.base.SecurityUtil@havePermission('/employee!update')">
										str += ez.formatString('<img class="iconImg ext-icon-note_edit" title="编辑" onclick="editFun(\'{0}\');"/>', row.id);
									</s:if>
									<s:if test="@it.pkg.util.base.SecurityUtil@havePermission('/employee!deleteByStatus')">
									...
									if (row.cards == "undefined" || row.cards == null || row.cards == undefined || row.cards == "") {
										<s:if test="@it.pkg.util.base.SecurityUtil@havePermission('/employee!saveArchivesOpenCard')">
					...
					<td>
						...
						<form id="searchForm">
							<table>
								<tr>
									<!-- 新增员工档案信息(权限控制) -->
									<s:if test="@it.pkg.util.base.SecurityUtil@havePermission('/employee!save')">
										<td>

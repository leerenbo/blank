	    ...
	    <script type="text/javascript">
			...
			function setCardType(obj){
				...
				if ( cardtype == "1" || cardtype == "2"  || cardtype == "3") {
					$("#PasmCardIndex").parent().css('display','block');
					$("#CpuCardType").parent().css('display','block');
				} else {
					$("#PasmCardIndex").parent().css('display','none');
					$("#CpuCardType").parent().css('display','none');
				}
			}
	    	...
	    	$(function (){
				...
				var comSize = $("#com option").length;
				for (var i = 0; i < comSize; i++) {
					if ($("#com").get(0).options[i].value == readCardComValue) {
						$("#com").combobox('select',readCardComValue);
						break;
					}
				}
				...
				var cardTypeSize = $("#cardtype option").length;
				for (var i = 0; i < cardTypeSize; i++) {
					if ($("#cardtype").get(0).options[i].value == cardTypeValue) {
						$("#cardtype").combobox('select',cardTypeValue);
						break;
					}
				}
				...
				if (cardTypeValue == "1" || cardTypeValue == "2" || cardTypeValue == "3") {
					...
					var pasmSize = $("#PasmCardIndex option").length;
					for (var i = 0; i < pasmSize; i++) {
						if ($("#PasmCardIndex").get(0).options[i].value == pasmCardIndexValue) {
							$("#PasmCardIndex").combobox('select',pasmCardIndexValue);
							break;  
						}
					}
					...
					var cpuSize = $("#CpuCardType option").length;
					for (var i = 0; i < cpuSize; i++) {
						if ($("#CpuCardType").get(0).options[i].value == cpuCardTypeValue) {
							$("#CpuCardType").combobox('select',cpuCardTypeValue);
							break;  
						}
					}
				} else {
				...
				var cyptModeSize = $("#cyptMode option").length;
				for (var i = 0; i < cyptModeSize; i++) {
					if ($("#cyptMode").get(0).options[i].value == m1EnModeValue) {
						$("#cyptMode").combobox('select',m1EnModeValue);
						break;  
					}
				}		
				/* -----------------初始化信息end----------------- */
				...
				$("#saveConfig").click(function(){
					var message = new Array(); //定义一数组
					var cardTypeValue = $("#cardtype").combobox('getValue');//卡类型
					var readCardComValue = $("#com").combobox('getValue');//读卡器串口号
					var cyptModeValue = $("#cyptMode").combobox('getValue');//加密方式
					...
					if (message[0] == 0) {
						...
						if (message[0] == 0) {
							...
							if ( cardTypeValue == "1" || cardTypeValue == "2" || cardTypeValue == "3" || cardTypeValue == "4") {//CPU卡
								...
								if (message[0] == 0) {
									...
									if (message[0] == 0) {
										...
										$.messager.alert('','保存CPU卡类型失败!','error');
									}
								} else {
									$.messager.alert('','保存PSAM卡槽失败!','error');
								}
							} else {
								$.messager.confirm('','保存成功，请关闭浏览器，重新进入系统!',function(r){
									if (r) {
							...
							$.messager.alert('','保存卡类型失败!','error');
						}
					} else {
						$.messager.alert('','保存串口配置失败!','error');
					}
				});
	        });
        </script>		

		...
		<script type="text/javascript">
			...
			$(function (){
				...
				grid = $('#grid').datagrid({
					...
					frozenColumns : [
						[ 
				   			<s:if test="@it.pkg.util.base.CookieUtil@haveKeyValue('easyuiStyle','check')">
				   			{	                    
				   				field:'ck',
				   				checkbox:true
				   			},
					...
					columns : [ 
						[   
							...
							<s:if test="@it.pkg.util.base.CookieUtil@haveKeyValue('easyuiStyle','icon')">
							,
							{
								title : '操作',
								field : 'action',
								...
								formatter : function(value, row){
									var str = '';
									if (row.id > 30000000) {
										<s:if test="@it.pkg.util.base.SecurityUtil@havePermission('/card!update')">
											str += ez.formatString('<img class="iconImg ext-icon-note_edit" title="编辑" onclick="editFun(\'{0}\');"/>', row.id);
										</s:if>
									}
									if (row.id > 30000000) {
										<s:if test="@it.pkg.util.base.SecurityUtil@havePermission('/card!deleteByStatus')">
									...
									if (row.status == '1' && row.id > 30000000) {
										<s:if test="@it.pkg.util.base.SecurityUtil@havePermission('/card!cardLoss')">
										...
										<s:if test="@it.pkg.util.base.SecurityUtil@havePermission('/card!cardRemoveLoss')">
											str += ez.formatString('<img class="iconImg ext-icon-key_add" title="解挂" onclick="cardRemoveLoss(\'{0}\');"/>', row.id);
										</s:if>
										<s:if test="@it.pkg.util.base.SecurityUtil@havePermission('/card!cardChange')">
				...
				function xiaohu(row){
					//如果是一卡通同步数据，禁止销户
					if (row.id <= 30000000) {
						$.messager.alert('提示', '一卡通同步数据，不允许销户', 'info');
						return;
					}
					parent.$.messager.confirm('询问', '您确定要销户['+row.employee.employeename+row.employee.employeenumber+']', function(r) {
						if (r) {
							$.post(
				...
				$("#cardLoss").click(function() {
					...
					var id = rows[0].id;
					if (id <= 30000000) {
						$.messager.alert('提示', '一卡通同步数据，不允许挂失', 'info');
						return;
					}
					//只有正常状态允许挂失
					var status = rows[0].status;
					if (status != "1") {
						$.messager.alert('提示', '卡状态非正常，不允许挂失', 'info');
						return;
					}
					$.post(
						...
						{id : id},
						function(result) {
							$("#cardKey").val(result.id);
							$("#employeeid").val(result.employee.id);
							$("#employeenumber").val(result.employee.employeenumber);
							$("#employeename").val(result.employee.employeename);
							showBtn("挂失", 1);
							$("#lossDialog").dialog('open');
						}, 
						'json'
					);
				});
				...
				$("#cardRemoveLoss").click(function() {
					...
					var id = rows[0].id;
					if (id <= 30000000) {
						$.messager.alert('提示', '一卡通同步数据，不允许解挂', 'info');
						return;
					}
					//只有挂失状态允许解挂
					var status = rows[0].status;
					if (status != "2") {
						$.messager.alert('提示', '卡状态非挂失，不允许解挂', 'info');
						return;
					}
					$.post(
						datalook.contextPath + '/card!getById.action', 
				...
				function lossSubmit(operateType) {
					if ($("#lossDialog form").form('validate')) {
						var url = "";
						if (operateType == 1) {
						...
						$("#cardLossBtn").linkbutton('disable');
						$.post(
							url, 
							$("#lossDialog form").serialize(), 
							function(result) {
								if (result.success) {
									$("#lossDialog").dialog('close');
									...
									$.messager.alert('提示', result.msg, 'error', function() {
										$("#lossDialog form :input:eq(3)").focus();
									});
								}
								$("#cardLossBtn").linkbutton('enable');
							}, 
							'json'
						);
					}
			...
			function removeFun(id, employeeid) {
				parent.$.messager.confirm('询问', '您确定要销户？', function(r) {
					if (r) {
						$.post(
							datalook.contextPath + '/card!deleteByStatus.action', 
							{
			...
			//换卡读卡
			$(function (){
				$("#cardChange_readCard").click(function(){
					//参数为空时，不判断卡序列号是否已被使用，直接读卡信息。参数为open时，判断卡序列号是否已经被使用
					var cardInfo = readCardInfo("open");
					if (cardInfo != null && cardInfo != undefined) {
						$("#cardChange_cardid").val(cardInfo.cardid);
					}
				});
			});
			...
			function cardChange(id){
				$.post(
					...
					{id : id},
					function(result) {
						$("#cardChange_cardKey").val(result.id);
						$("#cardChange_employeeid").val(result.employee.id);
						$("#cardChange_employeenumber").val(result.employee.employeenumber);
						$("#cardChange_employeename").val(result.employee.employeename);
						showChangeBtn("换卡");
						$("#cardChangeDialog").dialog('open');
					}, 
					'json'
				);
			}
			...
			function showChangeBtn(btnName) {
				$("#cardChangeDialog").show().dialog({
					...
					onOpen : function() {
						$("#cardChange_cardpwd").val("123456");
						$("#cardChange_cardid").val("");
						$("#cardChange_dumpCardpwd").val("123456");
						$("form :input").keyup(function(event) {
							if (event.keyCode == 13) {
								cardChangeSubmit();
							}
			...
			function cardChangeSubmit() {
				if ($("#cardChangeDialog form").form('validate')) {
					var url = datalook.contextPath + '/card!noSy_cardChange.action';
					$("#cardChangeBtn").linkbutton('disable');
					$.post(
						url, 
						$("#cardChangeDialog form").serialize(), 
						function(result) {
							if (result.success) {
								/* --------------写卡start-------------- */
								$.messager.progress({
								...
								var cardno = "00000000";
								var cardid = $("#cardChange_cardid").val();
								...
								if (res == "0") {
									...
									$.messager.alert('提示', '写卡失败，卡片不一致', 'error', function() {
										cancelOper(newId);
									});
								} else if (res == "2") {
									$.messager.alert('提示', '写卡失败，卡片初始化错误', 'error', function() {
										cancelOper(newId);
									});
								} else if (res == "3") {
									$.messager.alert('提示', '写卡失败，没有卡片', 'error', function() {
										cancelOper(newId);
									});
								} else if (res == "4") {
									$.messager.alert('提示', '写卡失败，非法的写卡组织信息', 'error', function() {
										cancelOper(newId);
									});
								} else if (res == "-1") {
									$.messager.alert('提示', '写卡失败，打开串口失败', 'error', function() {
										cancelOper(newId);
									});
								} else {
									$.messager.alert('提示', '写卡失败', 'error', function() {
										cancelOper(newId);
									});
								}
							} else {
								$.messager.alert('提示', result.msg, 'error', function() {
									$("#cardChangeDialog form :input:eq(4)").focus();
								});
							}
						}, 
						'json'
					);
					$("#cardChangeBtn").linkbutton('enable');
				}
			};
					...
					<td>
						...
						<form id="searchForm">
							<table>
								<tr>
									<!-- 开卡(权限控制) -->
									<s:if test="@it.pkg.util.base.SecurityUtil@havePermission('/card!save')">
										<td>
									...
									<s:if test="@it.pkg.util.base.CookieUtil@haveKeyValue('easyuiStyle','check')">
										...
										<s:if test="@it.pkg.util.base.SecurityUtil@havePermission('/card!cardChange')">
											<td>
												<a id="cardChange" onclick="var rows = $('#grid').datagrid('getChecked');if(rows.length>0){cardChange(rows[0].id)}" class="easyui-linkbutton" data-options="iconCls:'ext-icon-page_refresh',plain:true">换卡</a>
											</td>
										</s:if>
									</s:if>
		 							<td>
				...
				<fieldset>
					...
					<table class="table" style="width: 100%;">
						...
						<tr>
							...
							<th>重复卡密码</th>
							<td>
								<input id="cardChange_dumpCardpwd" type="password" class="easyui-validatebox" data-options="required:true,validType:'eqPwd[\'#cardChange_cardpwd\']'"/>
							</td>
						</tr>
					</table>
				</fieldset>

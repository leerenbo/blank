	...
	var save = function() {
		var dialog = parent.ez.modalDialog({
			...
			buttons : [ {
				text : '添加',
				handler : function() {
					dialog.find('iframe').get(0).contentWindow.submitForm(dialog, ezgrid, parent.$, parent.mainMenu);
				}
			} ]
		});
	};
	...
	var deletePhysical = function() {
		...
		parent.$.messager.confirm('询问', '您确定要删除记录？', function(r) {
			if (r) {
				...
				$.post(ez.contextPath + '/sysFunction!deletePhysical.action', {
					'ids' : ids.join(',')
				}, function() {
		...
		ezgrid = $('#ezgrid').treegrid({
			...
			columns : [ [ {
				...
				formatter : function(value,row){
					return ez.columnsFomatter(value,row,'it.pkg.model.sys.SysFunction.functiontype');
				}
			}, {
				width : '80',
				title : '排序',
...
<body class="easyui-layout" data-options="fit:true,border:false">
	<div id="eztoolbar" style="display: none;">
		<s:if test="@it.pkg.util.base.SecurityUtil@havePermission('/sysFunction!save')">
			<a onclick="save();" href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'ext-icon-note_add',plain:true">添加</a>
		</s:if>
		<s:if test="@it.pkg.util.base.SecurityUtil@havePermission('/sysFunction!update')">
		...
		<s:if test="@it.pkg.util.base.SecurityUtil@havePermission('/sysFunction!deletePhysical')">
